...
// --- Icons (Inline SVGs เพื่อความสะดวก ไม่ต้องลง Lib เพิ่ม) ---
...
export default function AddEmployeeForm({ brandId, onSuccess }: { brandId: string, onSuccess?: () => void }) {
  ...
  // สร้าง state สำหรับเช็คว่า field ไหนกำลังถูก focus (เพื่อให้เส้นขอบเปลี่ยนสีสวยๆ)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const fakeEmail = `${formData.username}@gmail.com`
      ...
      // ใช้ Toast แทน Alert จะดีกว่า (แต่ตอนนี้ใช้ Alert ไปก่อน)
  ...
  // Component ย่อยสำหรับ Input Field เพื่อลด Code ซ้ำและคุม Design System
  const InputGroup = ({ 
    ...
    <div className="group">
      ...
      >
        ...
        <div className="pr-4 pl-2 flex items-center">
            ...
            {/* ตัวนับสำหรับเบอร์โทร (แสดงเฉพาะเมื่อไม่มี suffix/toggle) */}
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {/* Button with Animation & Gradient */}
      <button 
        type="submit" 
