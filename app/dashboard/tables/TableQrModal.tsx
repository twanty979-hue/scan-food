...
const IconAlertTriangle = ({ size = 48 }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>;
...
interface TableQrModalProps {
    ...
    onClose: () => void;
    // ✅ เพิ่ม Props นี้ (Optional ก็ได้ เผื่อหน้าอื่นไม่ได้ส่งมา)
...
export default function TableQrModal({ table, brandId, brandSlug, qrLogoUrl, onClose, limitStatus }: TableQrModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] grid place-items-center p-4">
            <div className="absolute inset-0 bg-slate-900/85 backdrop-blur-sm cursor-pointer" onClick={onClose}></div>
            ...
            <div className="bg-white w-full max-w-md rounded-[40px] shadow-2xl relative z-10 overflow-hidden animate-in zoom-in duration-200">
                ...
                {/* Header */}
                <div className={`p-8 pb-14 text-center ${isLocked ? 'bg-red-500' : 'bg-slate-900'}`}>
                    <button onClick={onClose} className="absolute top-6 right-6 w-10 h-10 bg-white/10 rounded-full flex items-center justify-center text-white hover:bg-white/20 transition-all">
                        <IconX size={20} />
                    </button>
                ...
                <div className="relative -mt-10 px-8 pb-8 flex flex-col items-center bg-white rounded-t-[40px]">
                    ...
                    {isLocked ? (
                        <div className="flex flex-col items-center pt-10 pb-6 w-full text-center">
                            ...
                            <p className="text-slate-500 font-bold mb-6">
                                ...
                                <span className="text-red-500">(ใช้ไปแล้ว {limitStatus?.usage})</span>
                            </p>
                    ...
                    /* ✅ CASE 2: ปกติ -> โชว์ QR Code */
                        <>
                            ...
                            <div className="bg-slate-50 p-5 rounded-3xl mb-8 border border-slate-100 w-full text-center">
                                ...
                                <p className="text-4xl font-black text-slate-900 tracking-[0.4em] font-mono leading-none">{table.access_token}</p>
                            </div>
